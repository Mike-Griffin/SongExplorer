...
public struct NavigatableTabView: View {
...
@resultBuilder
enum TabBuilder {
    public static func buildBlock(_ items: NavigatableTabView.Tab...) -> [NavigatableTabView.Tab] {
        items
    }
}

extension View {
    public func tab(title: String, image: String? = nil,
                    selectedImage: String? = nil, badgeValue: String? = nil) -> NavigatableTabView.Tab {
        ...
        return NavigatableTabView.Tab(view: AnyView(self), barItem: barItem)
    }
}

private struct TabBarController: UIViewControllerRepresentable {
        ...
        private func popToRootOrScrollUp(on viewController: UIViewController) {
            ...
            if (popped ?? []).isEmpty {
                ...
                if let scrollView = firstScrollView(in: rootViewController.view ?? UIView()) {
                    ...
                    let yVal = -scrollView.adjustedContentInset.top
                    scrollView.setContentOffset(CGPoint(x: preservedX, y: yVal), animated: true)
                }
            }
        }
